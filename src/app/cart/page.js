...
export default function CartPage(){
  ...
  let subtotal=0;
  for (const p of cartProducts) {subtotal +=cartProductPrice(p);}
  ...
  async function proceedToCheckout(e){
    ...
    const promise = new Promise((resolve, reject) => {
      fetch('/api/checkout',{
        ...
        body:JSON.stringify({
          address, //conjunto de campos de la dirección
    ...
    <div className="mt-8 grid gap-8 grid-cols-2">
      <div>
        ...
        {cartProducts?.length > 0 &&(cartProducts.map((product,index)=> (
           <CartProduct key={`${product._id}-${Math.ceil(Math.random()*100000)}`} product={product} onRemove={removeCartProduct} index={index} />
        )))}
